...
function ListarEstados() {//função com lista de todos os estados

  const estados = [ //array com todos os estados do brasil abaixo
  ...
  return estados.map(estado =>//map inteira disponibilizando um obj estado do array
    <option
      key={estado.sigla}//indentificador unico basico
      value={estado.sigla}//valor q vai para servidor
      data-testid={estado.sigla}>
      {estado.nome} {/*texto estado e nome */}
    </option>
  );
